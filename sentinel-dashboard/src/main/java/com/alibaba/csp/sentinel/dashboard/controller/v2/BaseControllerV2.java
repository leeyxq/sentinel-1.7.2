...
public abstract class BaseControllerV2<T extends RuleEntity> {
    ...
    public Result<T> apiAddFlowRule(@RequestBody T entity) {
        ...
        try {
            ...
            getLogger().error("Failed to add flow rule", throwable);
            return Result.ofThrowable(-1, throwable);
        }
    ...
    public Result<T> apiUpdateFlowRule(@PathVariable("id") Long id,
        ...
        try {
            ...
            getLogger().error("Failed to update flow rule", throwable);
            return Result.ofThrowable(-1, throwable);
        }
        return Result.ofSuccess(entity);
    }
